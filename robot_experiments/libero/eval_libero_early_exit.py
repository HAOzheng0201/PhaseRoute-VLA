...
from a1.vla.dynamic_compute.telemetry import NullTelemetryLogger, SafeJSONLTelemetryLogger
...
class GenerateConfig:
    ...
    pretrained_checkpoint: Union[str, Path] = "model/libero_exit"
    ...
    reseed_each_episode: bool = False                # Optional paired-evaluation RNG control
    ...
    # PhaseRoute M1 telemetry is opt-in and does not affect the model output.
    telemetry_enabled: bool = False
    telemetry_output_path: Optional[str] = None
    telemetry_flush_every: int = 100

    # M2 phase cache is independent from M1 telemetry and remains opt-in.
    phase_cache_enabled: bool = False
    phase_cache_output_dir: Optional[str] = None
    phase_cache_summary_dtype: str = "float16"

    # M3 phase-aware depth routing.  This is opt-in and requires A1's normal
    # ExitController; the three modes form the controlled ablation matrix.
    phase_depth_enabled: bool = False
    phase_estimator_checkpoint: Optional[str] = None
    phase_depth_ablation: str = "joint"  # min_depth | threshold | joint
    phase_history_len: int = 8
    ...
    vision_bank_tokens: int = 144
    vision_min_tokens_per_crop: int = 4
    vision_aggregation_fail_open: bool = True
    vision_preserve_position_ids: bool = True

    # M4.7 learned EFA runtime. Mutually exclusive with the static M4 path.
    learned_vision_aggregation_checkpoint: Optional[str] = None
...
def validate_config(cfg: GenerateConfig) -> None:
    ...
    if cfg.telemetry_enabled:
        assert cfg.telemetry_output_path, "telemetry_output_path is required when telemetry is enabled"
        assert cfg.telemetry_flush_every > 0, "telemetry_flush_every must be positive"
    if cfg.phase_cache_enabled:
        assert cfg.phase_cache_output_dir, (
            "phase_cache_output_dir is required when phase cache is enabled"
        )
        assert cfg.phase_cache_summary_dtype in {"float16", "float32"}
    if cfg.phase_depth_enabled:
        assert cfg.phase_estimator_checkpoint, (
            "phase_estimator_checkpoint is required when phase depth is enabled"
        )
        assert Path(cfg.phase_estimator_checkpoint).is_file(), (
            f"phase estimator checkpoint not found: {cfg.phase_estimator_checkpoint}"
        )
        assert cfg.phase_depth_ablation in {"min_depth", "threshold", "joint"}
        assert cfg.phase_history_len > 0
    ...
    if cfg.rp_pep_enabled:
        ...
        assert cfg.exit_interval == 2, "RP-PEP v1 requires exit_interval=2"
        assert cfg.steps_per_stage == 1, "RP-PEP v1 requires steps_per_stage=1"
        assert cfg.threshold_type == "cosine", (
            "RP-PEP v1 requires the frozen cosine exit metric"
        )
        assert cfg.exit_dist == "exp" and float(cfg.exit_ratio) == 1.0, (
            "RP-PEP v1 requires the frozen exp/1.0 thresholds"
        )
        assert cfg.load_threshold and cfg.thresholds is None, (
            "RP-PEP v1 must read the frozen threshold JSON"
        )
    if cfg.vision_aggregation_enabled:
        assert cfg.vision_keep_tokens > 0
        assert cfg.vision_bank_tokens >= cfg.vision_keep_tokens
        assert cfg.vision_min_tokens_per_crop > 0
    if cfg.learned_vision_aggregation_checkpoint is not None:
        assert not cfg.vision_aggregation_enabled, (
            "static and learned vision aggregation are mutually exclusive"
        )
        assert Path(cfg.learned_vision_aggregation_checkpoint).is_file(), (
            "learned vision aggregation checkpoint not found: "
            f"{cfg.learned_vision_aggregation_checkpoint}"
        )
...
def initialize_exit_controller(generate_cfg, model,dataloader,device):
    cfg = generate_cfg
    original_exit_layers = tuple(model.get_all_exit_idx(cfg.exit_interval))
    productive_exit_plan = None
    exit_layers = original_exit_layers
    if cfg.rp_pep_enabled:
        if model.config.action_head != "flow_matching":
            raise ValueError("RP-PEP requires a flow_matching action head")
        if int(model.config.num_diffusion_inference_steps) != 10:
            raise ValueError("RP-PEP model must be configured for exactly 10 FM steps")
        productive_exit_plan = a1_fm10_rp_pep_plan(original_exit_layers)
        exit_layers = productive_exit_plan.eligible_exit_layers

    value_net = ActionValueNet(exit_list=list(exit_layers), exit_head=model.action_head, model=model,
                                interval=cfg.exit_interval, threshold_type=cfg.threshold_type,
                                productive_exit_plan=productive_exit_plan)
    exit_controller = ExitController(value_net, exit_id_list=list(exit_layers), steps_per_stage=generate_cfg.steps_per_stage,
    ...
    if model.config.action_head == "flow_matching":
        json_file_path = state_dict_path.with_name(
            f"exit_action_delta_matrix_{cfg.task_suite_name}_fm_steps{generate_cfg.action_head_flow_matching_inference_steps}.json"
        )
    else:
        json_file_path = state_dict_path.with_name(f"exit_action_delta_matrix_{cfg.task_suite_name}.json")
    print(f"**** json_file_path: {json_file_path}")

    thresholds_json_file_path = state_dict_path.with_name(
        f"exit_thresholds_{cfg.task_suite_name}_{cfg.exit_dist}_{cfg.exit_ratio}.json"
    )
    if productive_exit_plan is not None:
        if not thresholds_json_file_path.is_file():
            raise FileNotFoundError(
                f"RP-PEP requires frozen thresholds: {thresholds_json_file_path}"
            )
        with open(thresholds_json_file_path, "r") as f:
            frozen_thresholds = {
                int(layer): float(value) for layer, value in json.load(f).items()
            }
        selected_thresholds = productive_exit_plan.select_eligible_thresholds(
            frozen_thresholds,
            lower_is_easier=exit_controller.leq,
        )
        exit_controller._set_threshold_value(selected_thresholds)
        print(
            "**** RP-PEP loaded frozen thresholds without recalibration or overwrite: "
            f"{thresholds_json_file_path}"
        )
        gc.collect()
        torch.cuda.empty_cache()
        return exit_controller

    # Official early-exit checkpoints ship the calibrated action-delta matrix
    # as a sidecar JSON. Prefer it so evaluation does not reload the 34 GB
    # model.pt or require RLDS data merely to consume cached calibration.
    checkpoint = None
    values = None
    if not generate_cfg.thresholds and generate_cfg.load_threshold and json_file_path.exists():
        print(f"load values for threshold from json file {json_file_path}")
        with open(json_file_path, "r") as f:
            values = torch.tensor(json.load(f))
    elif not generate_cfg.thresholds:
        checkpoint = torch.load(state_dict_path, map_location="cpu")
        values = checkpoint.get("exit_action_delta_matrix")

    if not generate_cfg.thresholds:
        if generate_cfg.load_threshold and values is not None:
            exit_controller.set_threshold(cfg, model, dataloader, cfg.exit_ratio, cfg.llm_name, values)
        else:
            if dataloader is None:
                raise FileNotFoundError(
                    f"Missing cached calibration matrix {json_file_path}; "
                    "an RLDS calibration dataloader is required to regenerate it."
                )
...
def run_episode(
    ...
    # Reset environment
    env.reset()
    if hasattr(learnable_vision_aggregator, "reset_route_state"):
        learnable_vision_aggregator.reset_route_state()
    ...
    if cfg.vision_aggregation_enabled:
        vision_aggregation_config = StaticVisionAggregationConfig(
            enabled=True,
            keep_tokens=cfg.vision_keep_tokens,
            bank_tokens=cfg.vision_bank_tokens,
            min_tokens_per_crop=cfg.vision_min_tokens_per_crop,
            fail_open=cfg.vision_aggregation_fail_open,
    ...
    while t < max_steps + cfg.num_steps_wait:
        ...
        if len(action_queue) == 0:
                    ...
                    observability_kwargs.update(
                        {
                            ...
                            "telemetry_context": policy_call_context,
                        }
                    )
                    ...
                    observability_kwargs.update(
                        {
                            ...
                            "phase_cache_context": policy_call_context,
                        }
                    )
                if phase_depth_enabled:
                    observability_kwargs.update(
                        {
                            "phase_depth_runtime": phase_depth_runtime,
                            "phase_depth_context": policy_call_context,
                            "apply_phase_depth_plan": phase_depth_control_enabled,
                        }
                    )
            ...
            actions = get_vla_action(
                ...
                vision_aggregation_config=vision_aggregation_config,
...
def run_task(
    ...
    for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
        ...
        if cfg.reseed_each_episode:
            episode_seed = cfg.seed + task_id * 10_000 + episode_idx
            set_seed_everywhere(episode_seed)
            log_message(f"Episode seed: {episode_seed}", log_file)
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    if cfg.exit_layer_id is not None:
        ...
        state_dict_path = resource_path(cfg.pretrained_checkpoint, "model.pt")
        if model.config.action_head == "flow_matching":
            cache_path = state_dict_path.with_name(
                f"exit_action_delta_matrix_{cfg.task_suite_name}_fm_steps{cfg.action_head_flow_matching_inference_steps}.json"
            )
        else:
            cache_path = state_dict_path.with_name(f"exit_action_delta_matrix_{cfg.task_suite_name}.json")
        if cfg.rp_pep_enabled or cfg.thresholds or (cfg.load_threshold and cache_path.exists()):
            dataloader = None
            if cfg.rp_pep_enabled:
                print("**** RP-PEP uses the frozen threshold JSON; skip calibration dataloader.")
            else:
                print(f"**** use cached threshold calibration values: {cache_path}")
        else:
    ...
    if cfg.telemetry_enabled:
        telemetry_path = Path(cfg.telemetry_output_path)
        telemetry_path.parent.mkdir(parents=True, exist_ok=True)
        telemetry_logger = SafeJSONLTelemetryLogger(
            telemetry_path,
            flush_every=cfg.telemetry_flush_every,
        )
        log_message(f"PhaseRoute telemetry: {telemetry_path}", log_file)
    phase_cache_writer = NullPhaseCacheWriter()
    if cfg.phase_cache_enabled:
        phase_cache_writer = SafePhaseCacheWriter(
            Path(cfg.phase_cache_output_dir),
            summary_dtype=cfg.phase_cache_summary_dtype,
        )
        log_message(f"PhaseRoute M2 cache: {cfg.phase_cache_output_dir}", log_file)
    phase_depth_runtime = None
    if cfg.phase_depth_enabled:
        phase_depth_runtime = SafePhaseDepthRuntime(
            cfg.phase_estimator_checkpoint,
            device=device,
            eligible_exit_layers=tuple(exit_controller.exit_id_list),
            history_len=cfg.phase_history_len,
            fm_steps_per_exit=cfg.action_head_flow_matching_inference_steps,
            exit_policy_config=phase_exit_policy_config_for_ablation(
                cfg.phase_depth_ablation
            ),
        )
        log_message(
            "PhaseRoute M3 depth routing: "
            f"mode={cfg.phase_depth_ablation}, "
            f"checkpoint={cfg.phase_estimator_checkpoint}, "
            f"history={cfg.phase_history_len}",
            log_file,
        )
    ...
    if cfg.learned_vision_aggregation_checkpoint is not None:
        learned_vision_runtime = load_distilled_vision_aggregator(
            cfg.learned_vision_aggregation_checkpoint,
            device=device,
            expected_hidden_dim=model.config.d_model,
        )
        log_message(
            "PhaseRoute M4.7 learned vision aggregation: "
            f"checkpoint={learned_vision_runtime.checkpoint_path}, "
            f"tokens={learned_vision_runtime.config.output_tokens}",
            log_file,
        )
    ...
    if telemetry_logger.error_count:
        logger.warning(
            "Telemetry completed with %d errors; last error: %s",
            telemetry_logger.error_count,
            telemetry_logger.last_error,
        )
    phase_cache_writer.close()
    if phase_cache_writer.error_count:
        logger.warning(
            "Phase cache completed with %d errors; last error: %s",
            phase_cache_writer.error_count,
            phase_cache_writer.last_error,
        )
    if phase_depth_runtime is not None:
        log_message(
            "PhaseRoute M3 runtime: "

...
def get_vla_action(
    ...
    with torch.inference_mode():
        ...
        raw_proprio = np.asarray(obs["state"]).copy() if "state" in obs else None
       ...
       # 通过预处理器处理
        ...
        telemetry_start = time.perf_counter() if telemetry_enabled else None
        ...
        inference_time = (
            time.perf_counter() - telemetry_start

...
from typing import Any, Optional, Sequence, Tuple, List, NamedTuple, Callable, Mapping
import math
import logging
import random
...
from a1.vla.dynamic_compute.telemetry import TelemetryEventCallback, emit_telemetry_event
...
from a1.vla.dynamic_compute.vision_teacher_cache import (
    FlowMatchingTraceCallback,
    VisionTeacherFeatureCallback,
...
class OLMoWithLastOutput(NamedTuple):
    ...
    fm_pos_offset: Optional[torch.Tensor] = None
    """Effective valid prefix length after optional visual compaction."""
...
class AffordVLAEarlyExit(Molmo):
    ...
    def predict_actions_flow_matching(
        ...
        fm_trace_context: Optional[Mapping[str, Any]] = None,
    ):
        ...
        emit_flow_matching_trace(
            fm_trace_callback,
            {
                **dict(fm_trace_context or {}),
                "input_x": initial_x,
                "output_action": predicted_actions,
                "fm_steps": int(steps),
            },
        )
        return predicted_actions
    ...
    def forward(
        ...
        phase_signal_callback: Optional[PhaseSignalCallback] = None,
        vision_aggregation_config: Optional[StaticVisionAggregationConfig] = None,
        vision_teacher_feature_callback: Optional[
            VisionTeacherFeatureCallback
        ] = None,
        fm_trace_callback: Optional[FlowMatchingTraceCallback] = None,
        precomputed_projected_features: Optional[torch.Tensor] = None,
        learnable_vision_aggregator: Optional[nn.Module] = None,
        vision_instruction_summary: Optional[torch.Tensor] = None,
    ) -> OLMoWithLastOutput:
        """
        ...
        has_precomputed_vision = precomputed_projected_features is not None
        has_visual_input = has_image or has_precomputed_vision

        assert not (has_image and has_precomputed_vision), (
            "Cannot provide both images and precomputed_projected_features."
        )
        assert not (has_visual_input and input_embeddings is not None), (
            "Cannot provide both visual inputs and input embeddings."
        )
        assert not (has_visual_input and past_key_values is not None), (
            "Cached key and values should not be used with visual inputs."
        )
        if learnable_vision_aggregator is not None and vision_aggregation_config is not None:
            raise ValueError(
                "learnable_vision_aggregator and vision_aggregation_config are mutually exclusive"
            )
        ...
        if self.config.use_proprio:
            ...
            # Autocast may produce bf16 projector output while token embeddings
            # remain fp32.  Index assignment does not perform implicit dtype
            # promotion, so align explicitly without changing the fp32
            # inference path.
            x[:,proprio_token_idx[0],:] = proprio_features.squeeze().to(x.dtype)

        num_image: Optional[int] = None
        phase_visual_summary = None
        phase_visual_token_count = None
        if has_visual_input:
            ...
            if images is not None:
                ...
                image_features = precomputed_projected_features
                if image_features.ndim != 4:
                    raise ValueError(
                        "precomputed_projected_features must have shape [B, C, M, D]"
                    )
                if image_features.shape[0] != batch_size:
                    raise ValueError(
                        "precomputed projected-feature batch size does not match input_ids"
                    )
                if image_features.shape[-1] != self.config.d_model:
                    raise ValueError(
                        "precomputed projected-feature hidden size does not match d_model"
                    )
            num_image, num_patch = image_features.shape[1:3]
            assert image_input_idx.shape == (batch_size, num_image, num_patch)

            if vision_teacher_feature_callback is not None:
                emit_vision_teacher_features(
            ...
            if phase_signal_callback is not None:
                try:
                    phase_visual_summary, phase_visual_token_count = (
                        summarize_visual_embeddings(image_features, image_input_idx)
                    )
                except Exception:
                    phase_visual_summary = None
                    phase_visual_token_count = None

                # The causal phase plan must be installed before visual-width
                # routing so the current policy call, rather than the previous
                # one, decides whether EFA compression is safe.
                emit_phase_signal_summary(
                    phase_signal_callback,
                    {
                        "visual_summary": phase_visual_summary,
                        "visual_token_count": phase_visual_token_count,
                    },
                )

            original_image_input_idx = image_input_idx
            original_image_features = image_features
            original_seq_len = seq_len
            original_active_tokens = int((input_ids[0] != -1).sum().item())
            aggregation_enabled = (
                learnable_vision_aggregator is not None
                or (
                    vision_aggregation_config is not None
                    and vision_aggregation_config.enabled
                )
            )
            ...
            if aggregation_enabled:
                try:
                    if learnable_vision_aggregator is not None:
                        if vision_instruction_summary is None:
                            raise ValueError(
                                "vision_instruction_summary is required by the learnable aggregator"
                            )
                        aggregation_output = learnable_vision_aggregator(
                            original_image_features,
                            original_image_input_idx,
                        ...
                        preserve_position_ids = True
                        fail_open = False
                        aggregation_kind = getattr(
                            aggregation_output,
                            "aggregation_kind",
                            "learned",
                        )
                        route_metadata = {}
                        if hasattr(aggregation_output, "route"):
                            route_metadata = {
                                "vision_route": aggregation_output.route,
                                "full_token_fallback": bool(
                                    aggregation_output.full_token_fallback
                                ),
                                "phase_profile_name": (
                                    aggregation_output.profile_name
                                ),
                                "phase_profile_reason": (
                                    aggregation_output.profile_reason
                                ),
                                "phase_profile_uncertainty": getattr(
                                    aggregation_output,
                                    "profile_uncertainty",
                                    None,
                                ),
                                "full_token_trigger": getattr(
                                    aggregation_output,
                                    "full_token_trigger",
                                    None,
                                ),
                        ...
                        aggregated = aggregate_projected_vision(
                            original_image_features,
                            original_image_input_idx,
                            vision_aggregation_config,
                        )
                        preserve_position_ids = (
                            vision_aggregation_config.preserve_position_ids
                        )
                        fail_open = vision_aggregation_config.fail_open
                        aggregation_kind = "static"
                        route_metadata = {}
                    if aggregated.compression_applied:
                        compacted = compact_multimodal_sequence(
                            ...
                            preserve_position_ids=preserve_position_ids,
                        )
                        x = compacted.embeddings
                        input_ids = compacted.input_ids
                        attention_mask = compacted.attention_mask
                        attention_bias = compacted.attention_bias
                        response_mask = compacted.response_mask
                        subsegment_ids = compacted.subsegment_ids
                        position_ids = compacted.position_ids
                        image_input_idx = compacted.image_input_idx
                        proprio_token_idx = compacted.proprio_token_idx
                        append_last_valid_logits = compacted.append_last_valid_logits
                        seq_len = x.shape[1]

                        valid = aggregated.valid_mask
                        batch_idx = torch.arange(batch_size, device=x.device)
                        batch_idx = batch_idx[:, None].expand_as(
                            aggregated.sequence_positions
                        )
                        aggregated_features = aggregated.features.to(x.device)
                        mapped_positions = image_input_idx
                        x[batch_idx[valid], mapped_positions[valid]] += (
                            aggregated_features[valid]
                        )
                    else:
                        # Explicit keep-all follows the original insertion path
                        # exactly; this is the M4 regression-equivalence mode.
                        flat_features = original_image_features.view(
                            batch_size, num_image * num_patch, -1
                        )
                        flat_positions = original_image_input_idx.view(
                            batch_size, num_image * num_patch
                        )
                        valid = flat_positions >= 0
                        batch_idx = torch.arange(batch_size, device=x.device)
                        batch_idx = batch_idx[:, None].expand_as(flat_positions)
                        flat_features = flat_features.to(x.device)
                        x[batch_idx[valid], flat_positions[valid]] += flat_features[valid]
                    ...
                    emit_telemetry_event(
                        ...
                        "vision_aggregation",
                        {
                            "status": "compressed" if aggregated.compression_applied else "keep_all",
                            "aggregation_kind": aggregation_kind,
                            "source_projected_tokens": int(aggregated.source_counts.max().item()),
                            "original_tokens": int(aggregated.original_counts.max().item()),
                            "bank_tokens": int(aggregated.bank_counts.max().item()),
                            "kept_tokens": int(aggregated.kept_counts.max().item()),
                            "original_active_tokens": original_active_tokens,
                            "active_tokens": int((input_ids[0] != -1).sum().item()),
                            "original_llm_sequence_length": int(original_seq_len),
                            "llm_sequence_length": int(seq_len),
                            "original_tokens_per_crop": (
                                (original_image_input_idx[0] >= 0)
                                .sum(dim=-1)
                                .detach()
                                .cpu()
                                .tolist()
                            ),
                            "kept_tokens_per_crop": [
                                int(
                                    (
                                        aggregated.valid_mask[0]
                                        & (aggregated.crop_ids[0] == crop_id)
                                    ).sum().item()
                                )
                                for crop_id in range(num_image)
                            ],
                            "preserve_position_ids": preserve_position_ids,
                            **route_metadata,
                        },
                    ...
                    emit_telemetry_event(
                        telemetry_callback,
                        "vision_aggregation_error",
                        {
                            "error_type": type(error).__name__,
                            "message": str(error),
                            "fail_open": (
                                vision_aggregation_config.fail_open
                    ...
                    if (
                        vision_aggregation_config is None
                        or not vision_aggregation_config.fail_open
                    ):
                        raise

            if not aggregation_succeeded:
                # Original A1 insertion path, also used by fail-open recovery.
                image_features = original_image_features.view(
                    batch_size, num_image * num_patch, -1
                )
                image_input_idx = original_image_input_idx.view(
                    batch_size, num_image * num_patch
                )
                valid = image_input_idx >= 0
                batch_idx = torch.arange(batch_size, device=x.device)
                batch_idx = torch.tile(
                    batch_idx[:, None], [1, image_features.shape[1]]
                )
                image_features = image_features.to(x.device)
                x[batch_idx[valid], image_input_idx[valid]] += image_features[valid]

        if phase_signal_callback is not None and not has_visual_input:
            emit_phase_signal_summary(
                phase_signal_callback,
                {
                    "visual_summary": phase_visual_summary,
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                    ...
                    if exit_id == block_idx:
                        if telemetry_callback is not None:
                            emit_telemetry_event(
                                telemetry_callback,
                                "forced_exit",
                                {"layer_idx": block_idx, "exit_id": exit_id},
                            )
                        ...
                        return OLMoWithLastOutput(logits=None, attn_key_values=attn_key_values, last_hidden_state=x,exit_layer=block_idx,
                                ...
                                fm_pos_offset=(input_ids != -1).to(torch.int64).sum(dim=1))
                ...
                if exit_controller is not None:
                    ...
                    exit_controller_kwargs = {}
                    if fm_trace_callback is not None:
                        exit_controller_kwargs["fm_trace_callback"] = fm_trace_callback
                    if telemetry_callback is None:
                        exit_flag, exit_action = exit_controller(
                            feats_for_exit,
                            block_idx,
                            action_proprio,
                            start_idx,

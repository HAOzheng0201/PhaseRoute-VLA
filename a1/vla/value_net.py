...
from a1.vla.dynamic_compute.budget_profiles import ResolvedBudget
...
from a1.vla.dynamic_compute.productive_exit import ProductiveExitPlan
...
class ActionValueNet(BaseValueNet):
    def __init__(
        ...
        threshold_type,
        anchor=False,
        productive_exit_plan: Optional[ProductiveExitPlan] = None,
    ) -> None:
        super().__init__()
        ...
        self.anchor = anchor
        self.productive_exit_plan = productive_exit_plan
        if productive_exit_plan is not None:
            if anchor:
                raise ValueError("RP-PEP cannot be combined with anchor=True")
            if tuple(exit_list) != productive_exit_plan.eligible_exit_layers:
                raise ValueError("ActionValueNet exits do not match RP-PEP plan")
    ...
    def forward(  # type: ignore
        ...
        def predict_flow_matching(kvs, *, input_x=None, layer, role):
            kwargs = {}
            ...
            output = self.model.predict_actions_flow_matching(
                kvs,
                proprio,
                pos_offset,
                **kwargs,
            ...
            self.last_fm_steps += int(
                getattr(self.model.config, "num_diffusion_inference_steps", 0)
            )
            return output

        def burn_flow_matching_noise(kvs, count):
            if count <= 0:
                return
            if not kvs or not isinstance(kvs[0], (tuple, list)):
                raise ValueError("RP-PEP RNG burn requires flow-matching KV cache")
            first = kvs[0][0]
            shape = (
                first.shape[0],
                self.model.config.num_actions_chunk,
                self.model.config.fixed_action_dim,
            )
            for _ in range(count):
                torch.randn(shape, device=first.device, dtype=first.dtype)
            self.last_rng_burns += int(count)
        ...
        if mode == 'infer':
            ...
            productive_reference = (
                self.productive_exit_plan.comparison_reference(i)
                if self.productive_exit_plan is not None
                else None
            )
            if self.productive_exit_plan is not None:
                burn_flow_matching_noise(
                    feats,
                    self.productive_exit_plan.rng_burn_count(i),
                )
            if productive_reference is not None:
                if self.model.config.action_head != 'flow_matching':
                    raise ValueError("RP-PEP requires flow_matching action head")
                prev_action = predict_flow_matching(
                    feats[:productive_reference + 1],
                    layer=productive_reference,
                    role="comparison_reference",
                )
            elif len(self.action_list) == 0:
                ...
                if self.model.config.action_head != 'flow_matching':
                    ...
                    prev_action = predict_flow_matching(
                        feats[:prev_layer_idx+1],
                        layer=prev_layer_idx,
                        role="comparison_previous",
                    )
...
class ExitController(torch.nn.Module):
    def __init__(self, value_net, exit_id_list, steps_per_stage, exit_dist='exp', leq=True, max_layer=12):
        ...
        self.phase_exit_policy: Optional[PhaseAwareExitPolicy] = None
    ...
    def set_phase_plan(
        ...
        *,
        ...
        self.resolved_budget = budget
        self.phase_profile_reason = profile_reason

    def clear_phase_plan(self) -> None:
        self.phase_exit_policy = None
        self.phase_state = None
        self.resolved_budget = None
        self.phase_profile_reason = None

    @property
    def phase_plan_active(self) -> bool:
        values = (
            self.phase_exit_policy,
            self.phase_state,
            self.resolved_budget,
        )
        if any(value is None for value in values):
            if not all(value is None for value in values):
                raise RuntimeError("Incomplete phase exit plan")
            return False
        return True
    ...
    def forward(
        ...
        if self.cur_step % self.steps_per_stage != 0:
            reused_exit_id = getattr(self, "cur_exit_id", self.max_layer)
            if phase_active and self.phase_exit_policy.config.enforce_min_depth:
                reused_exit_id = max(
                    reused_exit_id,
                    self.resolved_budget.min_exit_layer,
                )
            exit_flag = i >= reused_exit_id
            if telemetry_callback is not None:
                emit_telemetry_event(
                    ...
                    "exit_candidate",
                    {
                        ...
                        "fm_steps": 0,
                        "phase_profile_id": (
                            self.resolved_budget.profile_id if phase_active else None
                        ),
                        "phase_profile_name": (
                            self.resolved_budget.profile.name if phase_active else None
                        ),
                        "phase_reason": (
                            "reused_stage_exit" if phase_active else None
                        ),
                    },
                )
            return exit_flag, None

        if (
            phase_active
            and self.phase_exit_policy.config.enforce_min_depth
            and exit_rank < self.resolved_budget.min_exit_rank
        ):
            if telemetry_callback is not None:
                emit_telemetry_event(
                    telemetry_callback,
                    "exit_candidate",
                    {
                        "layer_idx": i,
                        "evaluated": False,
                        "reused_stage_exit": False,
                        "should_exit": False,
                        "threshold_passed": None,
                        "forced_by_max_layer": False,
                        "action_delta": None,
                        "threshold": None,
                        "base_threshold": float(self.thresholds[i]),
                        "leq": self.leq,
                        "fm_calls": 0,
                        "fm_steps": 0,
                        "phase_profile_id": self.resolved_budget.profile_id,
                        "phase_profile_name": self.resolved_budget.profile.name,
                        "phase_profile_reason": self.phase_profile_reason,
                        "phase_reason": "below_min_depth",
                        "min_exit_layer": self.resolved_budget.min_exit_layer,
                    },
                )
            return False, None
        ...
        if isinstance(self.value_net, ActionValueNet):
            value_net_kwargs = {}
            if fm_trace_callback is not None:
                value_net_kwargs["fm_trace_callback"] = fm_trace_callback
            value, action = self.value_net(
                x, i, proprio, start_idx, end_idx, pos_offset, **value_net_kwargs
            )
        else:
            raise NotImplementedError
        ...
        base_thr = float(self.thresholds[i])
        thr = base_thr
        value = float(value.mean().item())
        forced_by_max_layer = i >= self.max_layer
        phase_decision = None
        if phase_active:
            phase_decision = self.phase_exit_policy.should_exit(
                layer_idx=i,
                exit_rank=exit_rank,
                metric_value=value,
                ...
                lower_is_easier=self.leq,
                is_final_exit=forced_by_max_layer,
            )
            thr = phase_decision.adjusted_threshold
            threshold_passed = phase_decision.threshold_passed
            exit_flag = phase_decision.should_exit
        else:
        ...
        if telemetry_callback is not None:
            ...
            fm_calls = int(getattr(self.value_net, "last_fm_calls", 1))
            fm_steps = int(
                getattr(
                    self.value_net,
                    "last_fm_steps",
            ...
            emit_telemetry_event(
                ...
                "exit_candidate",
                {
                    ...
                    "forced_by_max_layer": forced_by_max_layer,
                    "action_delta": value,
                    "threshold": thr,
                    "base_threshold": base_thr,
                    "leq": self.leq,
                    ...
                    "fm_steps": fm_steps,
                    "rng_burns": int(
                        getattr(self.value_net, "last_rng_burns", 0)
                    ),
                    "productive_exit_plan": (
                        self.value_net.productive_exit_plan.name
                        if getattr(self.value_net, "productive_exit_plan", None)
                        is not None
                        else None
                    ),
                    "comparison_reference_layer": (
                        self.value_net.productive_exit_plan.comparison_reference(i)
                        if getattr(self.value_net, "productive_exit_plan", None)
                        is not None
                        else None
                    ),
                    ...
                    "phase_profile_reason": (
                        self.phase_profile_reason if phase_active else None
                    ),
                    "phase_reason": (
                        phase_decision.reason if phase_decision is not None else None
                    ),
                    "min_exit_layer": (

...
if [[ "${arm_position}" != "${expected_arm}" || "${episode_index}" != "13" || "${seed}" != "20260826" ]]; then
  echo "Route-first pilot selection differs from the frozen alternating state-13 schedule." >&2
  exit 2
fi
...
run_dir="${RUN_DIR:-${output_root}/task${task_id}_arm${arm_position}_route_first_gpu${gpu_index}_${timestamp}}"
if [[ -e "${run_dir}" ]]; then
...
echo "[Stage9-Pilot-Route] task ${task_id}, state 13, arm ${arm_position}, GPU ${gpu_index} (${gpu_uuid})"
echo "[Stage9-Pilot-Route] output: ${run_dir}"
...
# The frozen active-run attestor consumes preflight.json.  Keep the pilot's
# explicitly named copy while binding both names to the same inode and bytes.
ln "${run_dir}/stage9_preflight.json" "${run_dir}/preflight.json"
...
env "${common_env[@]}" \
  ...
  "${python_bin}" "${repo_root}/scripts/run_route_first_active.py" \
    ...
    --physical-gpu-index "${gpu_index}" \
    --expected-gpu-uuid "${gpu_uuid}" \
    --preflight "${run_dir}/stage9_preflight.json" \
    --output-dir "${run_dir}" \
...
"${python_bin}" "${repo_root}/scripts/validate_route_first_stage9_pilot_arm.py" \
  "${run_dir}" --repo-root "${repo_root}"
echo "[Stage9-Pilot-Route] completed and sealed: ${run_dir}"

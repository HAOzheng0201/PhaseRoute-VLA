...
for index in "${!tasks[@]}"; do
  ...
  pids+=("$!")
  echo "started full-depth positive-control task=${task_id} physical_gpu=${gpu} pid=${pids[-1]}"
done
...
for index in "${!pids[@]}"; do
  ...
  if wait "${pids[$index]}"; then
    echo "complete full-depth positive-control task=${task_id}"
  else
    echo "failed full-depth positive-control task=${task_id}; inspect task${task_id}/stdout.log" >&2
    status=1
  fi
done
exit "${status}"

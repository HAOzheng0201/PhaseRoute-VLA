...
output_root="${2:-${repo_root}/reports/m3_joint_heldout_20260801}"
...
modes=(baseline joint baseline joint)
tasks=("${task_a}" "${task_a}" "${task_b}" "${task_b}")
pids=()
for gpu in 0 1 2 3; do
  mode="${modes[$gpu]}"
  task_id="${tasks[$gpu]}"
  run_dir="${output_root}/task${task_id}_${mode}"
  ...
  pids+=("$!")
  echo "started task=${task_id} mode=${mode} physical_gpu=${gpu} pid=${pids[-1]}"
done

status=0
for index in 0 1 2 3; do
  if wait "${pids[$index]}"; then
    echo "complete task=${tasks[$index]} mode=${modes[$index]}"
  else
    echo "failed task=${tasks[$index]} mode=${modes[$index]}" >&2
    status=1
  fi
done
exit "${status}"

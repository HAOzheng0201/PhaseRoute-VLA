...
route_result="${repo_root}/reports/m423_full_depth_challenge_20260803_v1/gpu0/1_early_exit/result.json"
...
declare -a gpu_uuids=()
declare -a orders=(
  "oracle_rts full_depth"
  "full_depth oracle_rts"
  "oracle_rts full_depth"
  "full_depth oracle_rts"
)
...
for gpu in "${gpus[@]}"; do
  (
    ...
    for policy in ${orders[$gpu]}; do
      ...
      if [[ "${policy}" == "oracle_rts" ]]; then
        env "${common_env[@]}" \
          "${python_bin}" \
          "${repo_root}/scripts/dynamic_compute/profile_m424_oracle_route_then_solve.py" \
          "${common_args[@]}" \
...
while true; do
  ...
  echo "progress completed_sessions=${completed}/8 alive_workers=${alive}"
  if [[ "${alive}" == "0" ]]; then
    break
  fi
  sleep 30

#!/usr/bin/env python3
"""Replay same-noise A1 candidates at layers 11/13/27 for one D2 shard."""
...
RESULT_SCHEMA_VERSION = "phase-route-vla.v3.d2-candidate-shard-result.v1"
SHARD_COUNT = 4
COLLECTION_SEED = 20260820
...
def _run(args: argparse.Namespace) -> None:
    ...
    if type(args.shard_index) is not int or not 0 <= args.shard_index < SHARD_COUNT:
        raise ValueError("V3-D2 shard index must be in 0..3")
    ...
    if len(context_records) != int(context_result["rows"]):
        raise ValueError("V3-D2 context record count differs")
    context_by_index = {
    ...
    if not assigned or set(assigned_indices) - set(context_by_index):
        raise PermissionError("V3-D2 shard assignment/context binding differs")
    ...
    for name in ("HF_HUB_OFFLINE", "TRANSFORMERS_OFFLINE"):
        if os.environ.get(name) != "1":
            raise PermissionError(f"V3-D2 replay requires {name}=1")
    if not torch.cuda.is_available():
        raise RuntimeError("V3-D2 candidate replay requires CUDA")
    ...
    expected_output = (
        REPO_ROOT
        / "reports"
        / "v3_d2_development_candidates"
        / f"shard{args.shard_index}"
    ).resolve()
    output = args.output_dir.resolve()
    incomplete = output.with_name(output.name + ".incomplete")
    if output != expected_output:
        raise PermissionError("V3-D2 candidate shard output path differs")
    if output.exists() or incomplete.exists():
        raise FileExistsError("V3-D2 refuses to overwrite candidate evidence")
    ...
    if any(parameter.requires_grad for parameter in model.parameters()):
        raise RuntimeError("V3-D2 frozen A1 unexpectedly requires gradients")
    ...
    with records_path_out.open("w", encoding="utf-8") as record_file:
        for ordinal, call in enumerate(assigned, start=1):
            ...
            if (
                ...
                raise PermissionError("V3-D2 context/source row binding differs")
            ...
            if after_hash != shared_hash:
                raise RuntimeError("V3-D2 shared FM input mutated during replay")
            ...
            if ordinal == 1 or ordinal % 25 == 0 or ordinal == len(assigned):
                print(
                    f"V3-D2 shard={args.shard_index} row={ordinal}/{len(assigned)} "
    ...
    if candidate_tensor.shape != (len(assigned), 3, 8, 7):
        raise RuntimeError("V3-D2 candidate tensor geometry differs")
    if shared_tensor.shape != (len(assigned), 8, 7):
        raise RuntimeError("V3-D2 shared input tensor geometry differs")
    payload = {
        "schema_version": D2_CANDIDATE_SCHEMA_VERSION,
        "role": D2_ROLE,
        "suite": D2_SUITE,
        "shard_index": args.shard_index,
        "shard_count": SHARD_COUNT,
        "candidate_layers": torch.tensor(D2_REPLAY_LAYERS, dtype=torch.long),
        ...
        "episode_index": torch.tensor(
            [call.episode_index for call in assigned], dtype=torch.long
        ),
    ...
    payload_path = incomplete / "development_candidates.pt"
    ...
    checks = {
        ...
        "only_development_v2_payloads_opened": len(opened_payloads) == len(assigned)
        and all(call.episode_index in range(12, 30) for call in assigned),
        ...
        "no_target_training_threshold_or_control": True,
    }
    passed = all(checks.values())
    result = {
        "status": "PASS_V3_D2_CANDIDATE_SHARD"
        if passed
        else "FAIL_V3_D2_CANDIDATE_SHARD",
    ...
    if not passed:
        raise RuntimeError("V3-D2 candidate shard failed one or more gates")
    incomplete.rename(output)
    print("PASS_V3_D2_CANDIDATE_SHARD", flush=True)
    ...
    try:
        ...
        if incomplete.is_dir() and not (incomplete / "abort.json").exists():
            (incomplete / "abort.json").write_text(
                json.dumps(
                    {
                        "status": "ABORT_V3_D2_CANDIDATE_SHARD",

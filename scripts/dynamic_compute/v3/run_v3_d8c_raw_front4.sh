...
output_root="${repo_root}/reports/v3_d8_fresh_raw"
log_root="${repo_root}/reports/v3_d8_fresh_raw_launch_logs"
...
if [[ -n "$(git -C "${repo_root}" status --porcelain=v1)" ]]; then
  echo "D8C raw collection requires a clean worktree" >&2
  exit 2
fi
if [[ -e "${output_root}" || -e "${log_root}" ]]; then
  echo "D8C refuses to reuse raw output or log roots" >&2
  exit 3
fi
...
for batch_start in 0 4 8; do
  ...
  for offset in 0 1 2 3; do
    ...
    env \
      ...
      "${python_bin}" \
      "${repo_root}/scripts/dynamic_compute/v3/collect_v3_d8c_fresh_task.py" \
      --task-id "${task_id}" \

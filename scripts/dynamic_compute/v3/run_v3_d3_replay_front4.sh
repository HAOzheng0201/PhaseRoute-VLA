...
raw_root="${repo_root}/reports/v3_d3_calibration_raw"
context_result="${repo_root}/reports/v3_d3_calibration_context/result.json"
output_root="${repo_root}/reports/v3_d3_calibration_candidates"
log_root="${repo_root}/reports/v3_d3_calibration_candidate_logs"
...
if [[ -n "$(git -C "${repo_root}" status --porcelain=v1)" ]]; then
  echo "V3-D3 candidate replay requires a clean worktree" >&2
  exit 2
fi
"${python_bin}" -c 'import json,sys; d=json.load(open(sys.argv[1])); assert d["status"] == "PASS_V3_D3_CONTEXT"' "${context_result}"
if [[ -e "${output_root}" || -e "${log_root}" ]]; then
  echo "V3-D3 refuses to reuse candidate output or log roots" >&2
  exit 3
fi
...
for shard in 0 1 2 3; do
  ...
  env \
    ...
    "${python_bin}" \
    "${repo_root}/scripts/dynamic_compute/v3/replay_v3_d3_candidates.py" \
    --raw-root "${raw_root}" \
    ...
    >"${console_log}" 2>&1 &
  pids+=("$!")
  echo "started D3 candidate shard=${shard} physical_gpu=${shard} pid=${pids[-1]} log=${console_log}"
done
...
while true; do
  ...
  echo "D3_candidate_replay alive_workers=${alive}"
  if [[ "${alive}" == "0" ]]; then
    break
  fi
  sleep 30
...
for shard in 0 1 2 3; do
  if wait "${pids[$shard]}"; then
    echo "completed D3 candidate shard=${shard}"
  else
    echo "failed D3 candidate shard=${shard}; inspect ${log_root}/shard${shard}.log" >&2
    failed=1
  fi
done
if (( failed != 0 )); then
...
echo "PASS_V3_D3_CANDIDATE_FRONT4"

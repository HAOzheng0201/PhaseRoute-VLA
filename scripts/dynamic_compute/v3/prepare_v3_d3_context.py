#!/usr/bin/env python3
"""Build V3-D3 past-only context from calibration raw caches."""
...
RESULT_SCHEMA_VERSION = "phase-route-vla.v3.d3-context-result.v1"
...
def _load_phase_estimator(path: Path) -> PhaseStateEstimator:
    if stream_sha256(path) != D2_PHASE_CHECKPOINT_SHA256:
        raise PermissionError("V3-D3 phase checkpoint SHA-256 differs")
    checkpoint = torch.load(path, map_location="cpu", weights_only=True)
    if checkpoint.get("schema_version") != PHASE_SCHEMA_VERSION:
        raise ValueError("V3-D3 PhaseStateEstimator schema differs")
    config = PhaseEstimatorConfig(**checkpoint["model_config"])
    if config != PhaseEstimatorConfig():
        raise ValueError("V3-D3 PhaseStateEstimator geometry differs")
...
def _stack(values: list[torch.Tensor], name: str) -> torch.Tensor:
    if not values:
        raise ValueError(f"V3-D3 context column is empty: {name}")
    return torch.stack(values).contiguous()


def _run(args: argparse.Namespace) -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES", "") not in ("", "-1"):
        raise PermissionError("V3-D3 context construction is CPU-only")
    validate_d3_prerequisites(REPO_ROOT)
    load_calibration_selection(REPO_ROOT)
    ...
    expected_output = (
        REPO_ROOT / "reports" / "v3_d3_calibration_context"
    ).resolve()
    output = args.output_dir.resolve()
    incomplete = output.with_name(output.name + ".incomplete")
    if output != expected_output:
        raise PermissionError("V3-D3 context output path differs")
    if output.exists() or incomplete.exists():
        raise FileExistsError("V3-D3 refuses to overwrite context evidence")
    ...
    for task_id in D3_TASK_IDS:
        ...
        if (
            result.get("status") != "PASS_V3_D3_RAW_TASK"
            ...
            or result.get("episode_indices") != list(D3_EPISODES)
            or result.get("completed_episodes") != len(D3_EPISODES)
            or result.get("claim_boundary", {}).get(
                "independent_test_v2_payload_opened"
            )
            is not False
        ):
            raise PermissionError(f"V3-D3 raw task {task_id} result differs")
        manifest_path = task_dir / "teacher_calls" / "manifest.jsonl"
        if stream_sha256(manifest_path) != result.get("teacher_manifest_sha256"):
            raise PermissionError(f"V3-D3 raw task {task_id} manifest SHA differs")
        task_calls = load_calibration_task_calls(
            task_dir, task_id=task_id, dataset_index_start=next_index
        )
        if len(task_calls) != result.get("teacher_cache_calls"):
            raise PermissionError(f"V3-D3 raw task {task_id} call count differs")
    ...
    with torch.inference_mode(), records_path.open("w", encoding="utf-8") as record_file:
        for ordinal, call in enumerate(calls, start=1):
            ...
            if instruction.shape != (3584,) or not np.isfinite(instruction).all():
                raise ValueError("V3-D3 instruction summary differs")
            ...
            if int(history.history_mask.sum()) != expected_history:
                raise RuntimeError("V3-D3 past-only history length differs")
            state = phase(
            ...
            if ordinal == 1 or ordinal % 100 == 0 or ordinal == len(calls):
                print(
                    f"V3-D3 context row={ordinal}/{len(calls)} "
    ...
    payload = {
        "schema_version": D3_CONTEXT_SCHEMA_VERSION,
        "role": D3_ROLE,
        "suite": D3_SUITE,
        "selection_sha256": D3_SELECTION_SHA256,
        "phase_checkpoint_sha256": D2_PHASE_CHECKPOINT_SHA256,
        "dataset_index": torch.tensor([call.dataset_index for call in calls]).long(),
        "task_id": torch.tensor([call.task_id for call in calls]).long(),
        "episode_index": torch.tensor([call.episode_index for call in calls]).long(),
        "call_ordinal": torch.tensor([call.call_ordinal for call in calls]).long(),
        "step_id": torch.tensor([call.step_id for call in calls]).long(),
        "behavior_exit_layer": torch.tensor(
            [call.behavior_exit_layer for call in calls]
        ).long(),
    ...
    payload_path = incomplete / "calibration_context.pt"
    ...
    checks = {
        ...
        "only_calibration_v2_payloads_opened": opened_payloads == len(calls)
        and all(call.episode_index in D3_EPISODES for call in calls),
        "all_100_task_episode_groups_have_calls": len(first_calls) == 100,
        ...
        "no_candidate_label_refit_threshold_shadow_or_control": True,
        "independent_test_payload_not_opened": True,
    }
    passed = all(checks.values())
    result = {
        "status": "PASS_V3_D3_CONTEXT" if passed else "FAIL_V3_D3_CONTEXT",
        ...
        "task_rows": {
            str(task): sum(call.task_id == task for call in calls)
            for task in D3_TASK_IDS
        },
        "episode_rows": {
            str(episode): episode_histogram[episode] for episode in D3_EPISODES
        },
    ...
    if not passed or result["source_worktree_dirty"]:
        raise RuntimeError("V3-D3 context failed one or more gates")
    incomplete.rename(output)
    print("PASS_V3_D3_CONTEXT", flush=True)

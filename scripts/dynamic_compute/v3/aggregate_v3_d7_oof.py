#!/usr/bin/env python3
"""Aggregate 18 immutable V3-D7 folds into reused-development evidence."""
...
CONTRACT_VALIDATION = Path("results/v3/v3_d7_epistemic_contract_validation.json")
...
FOLD_ROOT = Path("reports/v3_d7_development_oof_folds")
OUTPUT = Path("reports/v3_d7_development_oof")
RESULT_SCHEMA_VERSION = "phase-route-vla.v3.d7-reused-development-result.v1"
PAYLOAD_SCHEMA_VERSION = "phase-route-vla.v3.d7-reused-development-payload.v1"
...
def json_object(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise PermissionError(f"V3-D7 aggregate JSON must be an object: {path}")
    return dict(value)
...
def target_metrics(score: torch.Tensor, target: torch.Tensor) -> dict[str, Any]:
    if (
        ...
        raise PermissionError("V3-D7 metric geometry differs")
    return {
...
def range_distribution(value: torch.Tensor) -> dict[str, Any]:
    quantiles = torch.quantile(
        value.double(),
        torch.tensor([0.0, 0.5, 0.9, 0.95, 0.99, 1.0], dtype=torch.float64),
    )
    return {
        ...
        "quantiles": {
            name: float(item)
            for name, item in zip(("min", "p50", "p90", "p95", "p99", "max"), quantiles)
        },
        "rows_above_1e-6": int((value > D7_MIN_HEAD_RANGE).sum()),
        "fraction_above_1e-6": float((value > D7_MIN_HEAD_RANGE).double().mean()),
    }


def main() -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "-1":
        raise PermissionError("V3-D7 OOF aggregation is CPU-only")
    if git_output("status", "--porcelain=v1"):
        raise PermissionError("V3-D7 OOF aggregation requires clean worktree")
    load_d7_contract(REPO_ROOT)
    output = REPO_ROOT / OUTPUT
    incomplete = output.with_name(output.name + ".incomplete")
    if output.exists() or incomplete.exists():
        raise FileExistsError("V3-D7 refuses to overwrite aggregate evidence")
    ...
    if (
        ...
        != "PASS_V3_D7_EPISTEMIC_ENSEMBLE_CONTRACT_FROZEN"
        or validation.get("contract", {}).get("sha256") != D7_CONTRACT_SHA256
    ):
        raise PermissionError("V3-D7 contract validation differs")
    d5_path = REPO_ROOT / D5_FORMAL
    d6_path = REPO_ROOT / D6_FORMAL
    d5_result = json_object(d5_path)
    d6_result = json_object(d6_path)
    if (
        stream_sha256(d5_path) != D5_FORMAL_SHA256
        or stream_sha256(d6_path) != D6_FORMAL_SHA256
        or d5_result.get("status") != "NEGATIVE_V3_D5_DEVELOPMENT_GATE"
        or d6_result.get("status") != "NEGATIVE_V3_D6_DEVELOPMENT_SELECTION"
    ):
        raise PermissionError("V3-D7 locked D5/D6 comparison differs")

    dataset_result_path = REPO_ROOT / DATASET_RESULT
    if stream_sha256(dataset_result_path) != DATASET_RESULT_SHA256:
        raise PermissionError("V3-D7 aggregate dataset result SHA differs")
    ...
    if (
        ...
        raise PermissionError("V3-D7 aggregate dataset evidence differs")
    ...
    if (
        ...
        raise PermissionError("V3-D7 aggregate dataset payload differs")
    data = development_data_from_mapping(dataset)

    started = time.perf_counter()
    head_prediction = torch.full(
        (D7_HEAD_COUNT, data.rows, 2), float("nan"), dtype=torch.float64
    )
    score = torch.full((data.rows, 2), float("nan"), dtype=torch.float64)
    head_range = torch.full((data.rows,), float("nan"), dtype=torch.float64)
    ...
    for episode in D5_EPISODES:
        ...
        if (
            result.get("status") != "PASS_V3_D7_OOF_OUTER_FOLD"
            ...
            or result.get("d7_contract_sha256") != D7_CONTRACT_SHA256
            or result.get("d7_contract_validation_sha256")
            != CONTRACT_VALIDATION_SHA256
            or stream_sha256(payload_path) != result.get("payload_sha256")
        ):
            raise PermissionError(f"V3-D7 outer result differs: episode {episode}")
        ...
        if (
            fold.get("schema_version") != D7_OOF_SCHEMA_VERSION
            or fold.get("outer_episode") != episode
            or fold.get("fit_count") != D7_FITS_PER_OUTER
            or not torch.equal(indices, expected_indices)
            or fold["validation_head_prediction"].shape
            != (D7_HEAD_COUNT, indices.numel(), 2)
            or fold["validation_score"].shape != (indices.numel(), 2)
            or fold["validation_full_head_range"].shape != (indices.numel(),)
            or fold["selected_layer"].shape != (indices.numel() // 2,)
            or not isinstance(threshold, Mapping)
            or result.get("threshold") != threshold
        ):
            raise PermissionError(f"V3-D7 outer payload differs: episode {episode}")
        recomputed_full, recomputed_gripper, recomputed_range = ensemble_scores(
            fold["validation_head_prediction"]
        )
        recomputed_score = torch.stack((recomputed_full, recomputed_gripper), dim=1)
        if not torch.equal(recomputed_score, fold["validation_score"]) or not torch.equal(
            recomputed_range, fold["validation_full_head_range"]
        ):
            raise PermissionError("V3-D7 ensemble score semantics differ")
        head_prediction[:, indices] = fold["validation_head_prediction"]
        score[indices] = fold["validation_score"]
        head_range[indices] = fold["validation_full_head_range"]
        assigned[indices] += 1
        ...
        if (
            ...
            raise PermissionError("V3-D7 outer call assignment differs")
        ...
        fold_results[str(episode)] = {
            ...
            "threshold_feasible": bool(threshold["feasible"]),
            "elapsed_seconds": float(result["elapsed_seconds"]),
        }
        selected_lambdas[str(episode)] = float(result["selected_lambda"])
        thresholds[str(episode)] = dict(threshold)
    if (
        not torch.equal(assigned, torch.ones_like(assigned))
        or not bool(torch.isfinite(head_prediction).all())
        or not bool(torch.isfinite(score).all())
        or not bool(torch.isfinite(head_range).all())
        or bool((selected_layer == -1).any())
    ):
        raise PermissionError("V3-D7 aggregate OOF assignment differs")
        ...
        false_records.append(
            {
                ...
                "full_action_head_scores": head_prediction[:, row, 0].tolist(),
                "full_action_upper_score": float(score[row, 0]),
                "full_action_head_range": float(head_range[row]),
                "head0_gripper_score": float(score[row, 1]),
                "outer_runtime_threshold": thresholds[str(episode)]["runtime_threshold"],
                "action_consistency": bool(data.action_consistency[row]),
            }
        )

    infeasible_folds = sum(not bool(item["threshold_feasible"]) for item in fold_results.values())
    range_stats = range_distribution(head_range)
    gate_checks = {
        ...
        "all_outer_shrunk_thresholds_feasible": infeasible_folds == 0,
        "minimum_60_safe_clusters": summary.safe_clusters >= D5_MIN_SAFE_CLUSTERS,
        "minimum_10_percent_early_exit_calls": summary.early_exit_fraction
        >= D7_MIN_EARLY_FRACTION,
        "all_10_tasks_have_nonzero_early_exit_calls": all(
            value > 0 for value in summary.per_task_early_calls
        ),
        "false_safe_cluster_exact_ucb95_at_most_5_percent": summary.false_safe_ucb95
        <= D5_MAX_FALSE_SAFE_UCB95,
        "false_full_action_clusters_at_most_three": len(false_full_clusters)
        <= D7_MAX_FALSE_FULL_CLUSTERS,
        "false_gripper_calls_at_most_zero": int(false_gripper.sum()) == 0,
        "ensemble_non_degenerate_on_at_least_one_percent_rows": range_stats[
            "fraction_above_1e-6"
        ]
        >= D7_MIN_NONDEGENERATE_FRACTION,
    ...
    status = (
        ...
        else "NEGATIVE_V3_D7_REUSED_DEVELOPMENT_SELECTION"
    )
    metrics = {
        "full_action_epistemic_upper_score": target_metrics(
            score[:, 0], data.unsafe_target[:, 0]
        ),
        "head0_gripper_step_probability": target_metrics(
            score[:, 1], data.unsafe_target[:, 1]
        ),
    }
    ...
    d6_selection = d6_result["formal_selection"]
    result = {
        "status": status,
        ...
        "role": "development_v2_reused_for_D7_method_selection",
        "suite": "libero_10",
        "rows": data.rows,
        ...
        "fits_per_outer": D7_FITS_PER_OUTER,
        "total_model_fits": 18 * D7_FITS_PER_OUTER,
        "selected_lambdas": selected_lambdas,
        "thresholds": thresholds,
        "infeasible_outer_folds": infeasible_folds,
        "full_action_head_range": range_stats,
        "OOF_metrics": metrics,
        "selection": {
        ...
        "estimated_efficiency": {
            ...
            "five_head_latency_included": False,
            "measured_end_to_end_latency": False,
        },
        "locked_comparison": {
            "D5_formal_sha256": D5_FORMAL_SHA256,
            "D6_formal_sha256": D6_FORMAL_SHA256,
            "D5_early_exit_fraction": d5_selection["early_exit_fraction"],
            "D6_early_exit_fraction": d6_selection["early_exit_fraction"],
            "D7_early_exit_fraction": summary.early_exit_fraction,
            "D5_false_safe_clusters": d5_selection["false_safe_clusters"],
            "D6_false_safe_clusters": d6_selection["false_safe_clusters"],
            "D7_false_safe_clusters": summary.false_safe_clusters,
            "D5_false_gripper_calls": d5_selection["false_gripper_calls"],
            "D6_false_gripper_calls": d6_selection["false_gripper_calls"],
            "D7_false_gripper_calls": int(false_gripper.sum()),
        ...
        "input_sha256": {
            ...
            "D7_contract_validation": CONTRACT_VALIDATION_SHA256,
            "D5_formal": D5_FORMAL_SHA256,
            "D6_formal": D6_FORMAL_SHA256,
            "dataset_result": DATASET_RESULT_SHA256,
            "dataset_payload": DATASET_PAYLOAD_SHA256,
            "outer_fold_results": {
                episode: item["sha256"] for episode, item in fold_results.items()
            },
        },
        "outer_folds": fold_results,
        "access_ledger": {
            "development_v2_payload_reopened_after_D5_D6_analysis": True,
            "development_model_fits": 18 * D7_FITS_PER_OUTER,
        ...
        "next_stage": {
            "authorized": (
                ...
                if status == "PROMISING_V3_D7_REUSED_DEVELOPMENT_SELECTION"
                else "D7_NEGATIVE_RESULT_ANALYSIS_ONLY"
            ),
        ...
        "claim_boundary": {
            ...
            "result_is_unbiased_D5_D6_comparison": False,
    ...
    payload_path = incomplete / "development_epistemic_nested_oof.pt"
    torch.save(
        {
            "schema_version": PAYLOAD_SCHEMA_VERSION,
            "contract_sha256": D7_CONTRACT_SHA256,
            ...
            "OOF_head_prediction": head_prediction.clone(),
            "OOF_score": score.clone(),
            "full_action_head_range": head_range.clone(),
            ...
            "development_data_reused_after_D5_D6_analysis": True,

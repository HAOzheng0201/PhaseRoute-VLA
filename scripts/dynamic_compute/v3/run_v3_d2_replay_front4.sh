...
raw_root="${repo_root}/reports/v3_d2_development_raw"
context_result="${repo_root}/reports/v3_d2_development_context/result.json"
output_root="${repo_root}/reports/v3_d2_development_candidates"
log_root="${repo_root}/reports/v3_d2_development_candidate_logs"
...
if [[ -n "$(git -C "${repo_root}" status --porcelain=v1)" ]]; then
  echo "V3-D2 candidate replay requires a clean worktree" >&2
  exit 2
fi
"${python_bin}" -c 'import json,sys; d=json.load(open(sys.argv[1])); assert d["status"] == "PASS_V3_D2_CONTEXT"' "${context_result}"
if [[ -e "${output_root}" || -e "${log_root}" ]]; then
  echo "V3-D2 refuses to reuse candidate output or log roots" >&2
  exit 3
fi
...
for shard in 0 1 2 3; do
  ...
  env \
    ...
    "${python_bin}" \
    "${repo_root}/scripts/dynamic_compute/v3/replay_v3_d2_candidates.py" \
    --raw-root "${raw_root}" \
...
echo "PASS_V3_D2_CANDIDATE_FRONT4"

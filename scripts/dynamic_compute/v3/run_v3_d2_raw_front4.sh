...
output_root="${repo_root}/reports/v3_d2_development_raw"
log_root="${repo_root}/reports/v3_d2_development_launch_logs"
...
if [[ -n "$(git -C "${repo_root}" status --porcelain=v1)" ]]; then
  echo "V3-D2 raw collection requires a clean worktree" >&2
  exit 2
fi
if [[ -e "${output_root}" ]]; then
  echo "Refusing to reuse ${output_root}" >&2
  exit 3
fi
if [[ -e "${log_root}" ]]; then
  echo "Refusing to reuse ${log_root}" >&2
  exit 4
fi
...
for batch_start in 0 4 8; do
  ...
  for offset in 0 1 2 3; do
    ...
    env \
      ...
      "${python_bin}" \
      "${repo_root}/scripts/dynamic_compute/v3/collect_v3_d2_development_task.py" \
      --task-id "${task_id}" \
    ...
    pids+=("$!")
    batch_tasks+=("${task_id}")
    echo "started task=${task_id} physical_gpu=${gpu} pid=${pids[-1]} log=${console_log}"
  done
  ...
  for index in "${!pids[@]}"; do
    ...
    if wait "${pids[$index]}"; then
      ...
      echo "failed task=${task_id}; inspect ${log_root}/task${task_id}.log" >&2
      failed=1
    fi
  done
  if (( failed != 0 )); then

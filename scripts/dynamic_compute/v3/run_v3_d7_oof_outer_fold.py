#!/usr/bin/env python3
"""Run one immutable outer episode fold of V3-D7 nested OOF."""
...
RESULT_SCHEMA_VERSION = "phase-route-vla.v3.d7-oof-outer-result.v1"
...
def json_object(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise PermissionError("V3-D7 outer metadata must be an object")
    return dict(value)


def run(args: argparse.Namespace) -> None:
    if os.environ.get("CUDA_VISIBLE_DEVICES") != "-1":
        raise PermissionError("V3-D7 nested OOF is CPU-only")
    if git_output("status", "--porcelain=v1"):
        raise PermissionError("V3-D7 nested OOF requires a clean worktree")
    if args.outer_episode not in D5_EPISODES:
        raise PermissionError("V3-D7 outer episode differs")
    ...
    if (
        ...
        raise PermissionError("V3-D7 contract validation evidence differs")
    ...
    if result_path != expected_result or stream_sha256(result_path) != DATASET_RESULT_SHA256:
        raise PermissionError("V3-D7 dataset result path or SHA differs")
    ...
    if (
        ...
        raise PermissionError("V3-D7 dataset result semantics differ")
    payload_path = result_path.parent / str(result["payload"])
    if stream_sha256(payload_path) != DATASET_PAYLOAD_SHA256:
        raise PermissionError("V3-D7 dataset payload SHA differs")
    ...
    if (
        ...
        raise PermissionError("V3-D7 dataset payload semantics differ")
    ...
    expected_output = (
        REPO_ROOT
        / "reports/v3_d7_development_oof_folds"
        / f"episode{args.outer_episode}"
    ).resolve()
    output = args.output_dir.resolve()
    incomplete = output.with_name(output.name + ".incomplete")
    if output != expected_output:
        raise PermissionError("V3-D7 outer output path differs")
    if output.exists() or incomplete.exists():
        raise FileExistsError("V3-D7 refuses to overwrite outer-fold evidence")
    ...
    if (
        ...
        or fold["validation_head_prediction"].shape[0] != D7_HEAD_COUNT
    ):
        raise RuntimeError("V3-D7 outer fold output semantics differ")
    fold_path = incomplete / "outer_fold.pt"
    torch.save(fold, fold_path)
    threshold = fold["threshold"]
    head_range = fold["validation_full_head_range"].double()
    outer_result = {
        "status": "PASS_V3_D7_OOF_OUTER_FOLD",
        ...
        "threshold": threshold,
        "outer_head_fit_rows": fold["outer_head_fit_rows"],
        "validation_head_range": {
            "minimum": float(head_range.min()),
            "mean": float(head_range.mean()),
            "maximum": float(head_range.max()),
            "rows_above_1e-6": int((head_range > 1.0e-6).sum()),
        },
        ...
        "d7_contract_validation_sha256": CONTRACT_VALIDATION_SHA256,
        "payload": fold_path.name,
        "payload_sha256": stream_sha256(fold_path),
        "elapsed_seconds": time.perf_counter() - started,
        "checks": {
            "authenticated_D5_dataset_D7_contract_and_validation_current": True,
            "outer_episode_excluded_from_all_five_heads_and_selection": True,
            "inner_episode_excluded_from_all_five_inner_heads": True,
            "fixed_episode_delete_group_geometry_used": True,
            "seventeen_inner_episode_folds_and_170_task_cells_exact": True,
            "full_action_max_and_head0_gripper_semantics_used": True,
            "fixed_0_95_shrink_without_reoptimization": True,
    ...
    print(
        "PASS_V3_D7_OOF_OUTER_FOLD "
        f"episode={args.outer_episode} lambda={fold['selected_lambda']} "
        f"threshold={threshold['runtime_threshold']}",
        flush=True,
    )
    ...
    try:
        ...
        if incomplete.is_dir() and not (incomplete / "abort.json").exists():
            (incomplete / "abort.json").write_text(
                json.dumps(
                    {
                        "status": "ABORT_V3_D7_OOF_OUTER_FOLD",

#!/usr/bin/env python3
"""Collect one V3-D3 calibration task over frozen episodes 30--39."""
...
RESULT_SCHEMA_VERSION = "phase-route-vla.v3.d3-raw-task-result.v1"
...
def _run(args: argparse.Namespace) -> None:
    contract_audit = validate_d3_prerequisites(REPO_ROOT)
    selection = load_calibration_selection(REPO_ROOT)
    task_window = task_calibration_window(selection, args.task_id)
    expected_output = (
        REPO_ROOT / "reports" / "v3_d3_calibration_raw" / f"task{args.task_id}"
    ).resolve()
    output = args.output_dir.resolve()
    incomplete = output.with_name(output.name + ".incomplete")
    if output != expected_output:
        raise PermissionError("V3-D3 raw task output path differs")
    if output.exists() or incomplete.exists():
        raise FileExistsError("V3-D3 refuses to overwrite raw task evidence")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("V3-D3 raw collection requires CUDA")
    ...
    if checkpoint == frozen_source:
        raise PermissionError("V3-D3 cannot use writable-sidecar source model")
    model_audit = validate_runtime_model_directory(
        checkpoint, args.model_attestation
    )
    base_suite = benchmark.get_benchmark_dict()[D3_SUITE]()
    if not 0 <= args.task_id < base_suite.n_tasks:
        raise ValueError("V3-D3 task id is outside LIBERO-Long")
    all_states = base_suite.get_task_init_states(args.task_id)
    if len(all_states) < 40:
        raise RuntimeError("V3-D3 LIBERO-Long task has fewer than 40 states")
    initial_state_sha256 = {
        str(record.episode_index): sha256_array(all_states[record.episode_index])
        for record in task_window
    }
    suite = CalibrationInitialStateWindowTaskSuite(base_suite)
    ...
    cfg = GenerateConfig(
        ...
        run_id_note=f"v3-d3-calibration-task{args.task_id}",
    ...
    if threshold_after != threshold_before:
        raise RuntimeError("V3-D3 model initialization changed thresholds")
    if stream_sha256(threshold_path) != D2_EXIT_THRESHOLDS_SHA256:
        raise RuntimeError("V3-D3 model initialization changed threshold bytes")
    ...
    def run_episode_with_global_identity(*call_args: Any, **call_kwargs: Any):
        if len(call_args) > 10:
            ...
            mutable[10] = global_calibration_episode_index(int(mutable[10]))
            return original_run_episode(*tuple(mutable), **call_kwargs)
        mutable_kwargs = dict(call_kwargs)
        mutable_kwargs["episode_idx"] = global_calibration_episode_index(
    ...
    checks = {
        "d3_contract_and_d2_model_current": bool(contract_audit),
        "calibration_selection_sha_exact": D3_SELECTION_SHA256
        == contract_audit["calibration_selection_sha256"],
        "runtime_checkpoint_attested_and_sidecars_local": bool(model_audit),
        "gpu_is_one_verified_physical_front_four_device": True,
        "all_10_selected_episodes_completed": episodes == len(task_window),
        ...
        "all_npz_shards_present": bool(shards)
        and all(path.is_file() for path in shards),
        "observer_writes_error_free": telemetry.error_count == 0
        and teacher.error_count == 0,
        "independent_test_payload_not_opened": True,
        "no_refit_threshold_selection_shadow_or_active_control": True,
    }
    passed = all(checks.values()) and not bool(source_status)
    result = {
        "status": "PASS_V3_D3_RAW_TASK" if passed else "FAIL_V3_D3_RAW_TASK",
        "schema_version": RESULT_SCHEMA_VERSION,
        "timestamp_utc": datetime.now(timezone.utc).isoformat(timespec="milliseconds"),
        "d3_contract_status": D3_STATUS,
        "d3_contract_sha256": D3_CONTRACT_SHA256,
    ...
    if not passed:
        raise RuntimeError("V3-D3 raw task failed one or more gates")
